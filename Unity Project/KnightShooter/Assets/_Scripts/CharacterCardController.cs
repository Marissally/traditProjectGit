...
public class CharacterCardController : MonoBehaviour {
    ...
    public Image shield;
    private variableTracker gameController;
    private CharacterController2D characterController;

	// Use this for initialization
	void Start () {
        gameController = GameObject.Find("KeptVariables").GetComponent<variableTracker>();
        characterController = GameObject.Find("Player 1").GetComponent<CharacterController2D>();
	}
	
	// Update is called once per frame
	void Update () {
		if (gameController.P1wins == 1 && wins1.enabled == false)
        {
            wins1.enabled = true;
        }

        if (gameController.P1wins == 2 && wins2.enabled == false)
        {
            wins2.enabled = true;
        }

        if (gameController.P1wins == 3 && wins3.enabled == false)
        {
            wins3.enabled = true;
        }

        if (gameController.P1wins == 0)
        {
        ...
        if (characterController.shotType == "Default")
        {
